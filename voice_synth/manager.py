...
from voice_synth.audio.devices import list_output_devices
from voice_synth.audio.playback import PlaybackQueue
...
from voice_synth.providers import (
    TTSProvider,
...
class TTSManager:
    ...
    voice-synth. It loads configured providers, selects an available provider
    ...
    def __init__(
        ...
        *,
        ...
        resolved_route_config = self.settings.voice_synth.route_config
        ...
        self._route_engine = _RoutePlaybackEngine(
            ...
            route_config_provider=lambda: self.settings.voice_synth.route_config,
        )
        self._cache = PhraseCache(self.settings.voice_synth.cache.path)
    ...
    def refresh_audio_devices(
        ...
        *,
        ...
        if use_system_defaults:
            self.settings.voice_synth.route_config = RouteConfig.default()
        route_config = self.settings.voice_synth.route_config
    ...
    def _get_provider(self, provider: str | None) -> TTSProvider:
        ...
        if configured_chain:
            raise ConfigurationError(
                "None of the configured providers are available. Update voice_synth.provider_chain or configure the missing backends."
            )
    ...
    def _resolve_default_provider(self) -> None:
        ...
        if self.settings.voice_synth.default_provider:
            return

        for provider_name in self.settings.voice_synth.provider_chain:
            provider_candidate = self._providers.get(provider_name)
            if provider_candidate is not None and provider_candidate.is_available():
                self.settings.voice_synth.default_provider = provider_name
                return
    ...
    def _provider_chain(self) -> list[str]:
        ...
        if self.settings.voice_synth.default_provider:
            return [
                self.settings.voice_synth.default_provider,
                *(
                    provider
                    for provider in self.settings.voice_synth.provider_chain
                    if provider != self.settings.voice_synth.default_provider
                ),
            ]
        return self.settings.voice_synth.provider_chain
