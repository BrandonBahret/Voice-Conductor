"""Private notebook display helpers for the voice_synth showcase."""
...
def build_word_timing_transcript_html(
    ...
    *,
    ...
    if not word_timing:
        return (
            "<div class=\"voice-synth-word-sync voice-synth-word-sync--missing\">"
            "Word timing metadata is unavailable for this audio."
            "</div>"
        )

    container_id = f"voice-synth-word-sync-{uuid4().hex}"
    duration_seconds = max(audio.duration_seconds, float(word_timing[-1]["end_seconds"]))
    word_spans = " ".join(
        (
            f"<span class=\"voice-synth-word\""
    ...
    label_html = (
        f"<div class=\"voice-synth-word-sync__label\">{escape(label)}</div>"
        if label
        else ""
    )

    return f"""
<div id="{container_id}" class="voice-synth-word-sync" data-duration="{duration_seconds:.6f}">
  <style>
    #{container_id} {{
    ...
    #{container_id} .voice-synth-word-sync__label {{
      margin-bottom: 0.35rem;
      color: #cbd5e1;
      font-size: 0.9rem;
    }}
    #{container_id} .voice-synth-word-sync__line {{
      font-size: 1.05rem;
      color: inherit;
      white-space: normal;
      font-weight: 500;
    }}
    #{container_id} .voice-synth-word {{
    ...
    #{container_id} .voice-synth-word.is-active {{
      background: #fde047;
      color: #1f2937;
  ...
  {label_html}
  <div class="voice-synth-word-sync__line">{word_spans}</div>
</div>
<script>
(() => {{
  ...
  const words = Array.from(root.querySelectorAll(".voice-synth-word"));
  if (!words.length) {{
    return;
  }}
